...
def Record(timestamp, lat, lon, id=b"0", speed=0.0):
    if not isinstance(timestamp, dt.datetime):
        timestamp = dt.datetime.strptime(timestamp, "%Y-%m-%d %H:%M:%S %Z").replace(
            tzinfo=pytz.utc
        )
...
def ResampledRecord(timestamp, lat, lon, point_density=1.0, id=b"0", speed=0.0):
    ...
    return resample.ResampledRecord(
        id=id, timestamp=timestamp, lat=lat, lon=lon, speed=speed, point_density=point_density
    )
...
expected = [
    ...
    ResampledRecord(
        "2011-07-01 02:30:00 UTC", 11.015713484026367, 11.01600758851356, 1.00, speed=10
    ),
    ResampledRecord(
        "2011-07-01 02:40:00 UTC",
        11.035355339059327,
        11.036017074155552,
        speed=10,
        point_density=0.56,
    ...
    ResampledRecord(
        "2011-07-01 02:50:00 UTC",
        lat=11.054997194092287,
        lon=11.056026559797516,
        speed=10,
        point_density=0.36,
    ),
    ResampledRecord(
        "2011-07-01 03:00:00 UTC",
        lat=11.074639049125247,
        lon=11.07603604543948,
        speed=10,
        point_density=0.26,
    ),
    ResampledRecord(
        "2011-07-01 03:10:00 UTC",
        lat=11.094280904158206,
        lon=11.096045531081444,
        speed=10,
        point_density=0.21,
    ),
    ResampledRecord(
        "2011-07-01 03:20:00 UTC",
        lat=11.113922759191166,
        lon=11.116055016723408,
        speed=10,
        point_density=0.17,
    ),
]
