...
def build():
    ...
    builder.add(
        "start_time", "TIMESTAMP", description="The start timestamp of the encounter (UTC)."
    )
    ...
    builder.add(
        ...
        "FLOAT",
        description=(
            "The mean latitude. Calculated in two stages. "
            "First these values are calculated for the raw segments when these values "
            "are computed as means over the interpolated points. "
    ...
    builder.add(
        ...
        "FLOAT",
        description=(
            "The mean longitude. Calculated in two stages. "
            "First these values are calculated for the raw segments when these values are computed "  # noqa
    ...
    builder.add(
        "start_lat", "FLOAT", description="The beginning latitude where the encounter takes place."
    )
    builder.add(
        "start_lon",
        "FLOAT",
        description="The beginning longitude where the encounter takes place.",
    )
    ...
    for v in [1, 2]:
        ...
        builder.add(
            ...
            "STRING",
            mode="REPEATED",
            description=f"The segment id of the vessel {v}.",
        )
        builder.add(
            f"vessel_{v}_point_count",
            "INTEGER",
            description=f"The amount of the points of contact during encounter by vessel {v}.",
        )
...
def build_raw_encounter():
    ...
    for v in [1, 2]:
        builder.add(
            f"vessel_{v}_seg_id", "STRING", description=f"The segment id of the vessel {v}."
        )
        builder.add(
            f"vessel_{v}_point_count",
            "INTEGER",
