    ...
    try:
        if isinstance(expected, (list, tuple)):
            return len(actual) == len(expected) and all(
                _approx_equal(a, b, fp_abs_tol, fp_rel_tol) for (a, b) in zip(expected, actual)
            )
        if isinstance(expected, dict):
            return sorted(actual.keys()) == sorted(expected.keys()) and all(
                _approx_equal(expected[k], actual[k], fp_abs_tol, fp_rel_tol) for k in expected
            )
        if isinstance(expected, float) or isinstance(actual, float):
            return (
                (math.isinf(actual) and math.isinf(expected))
                or (math.isnan(actual) and math.isnan(expected))
                or abs(actual - expected)
                <= max(fp_abs_tol, 0.5 * fp_rel_tol * (abs(actual) + abs(expected)))
            )
    except Exception:
