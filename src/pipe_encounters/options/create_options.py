...
class CreateOptions(PipelineOptions):
    ...
    def _add_argparse_args(cls, parser):
        ...
        required.add_argument(
            "--raw_table", required=True, help="Table to write raw (unmerged) encounters to"
        )
        required.add_argument(
            "--start_date", required=True, help="First date to look for entry/exit events."
        )
        required.add_argument(
            "--end_date",
            required=True,
        ...
        optional.add_argument(
            ...
            " the new data, or WRITE_APPEND to add the new date without. Defaults to WRITE_APPEND.",  # noqa
        )
        optional.add_argument(
