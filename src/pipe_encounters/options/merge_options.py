...
class MergeOptions(PipelineOptions):
    ...
    def _add_argparse_args(cls, parser):
        ...
        required.add_argument(
            "--raw_table", required=True, help="Table to pull raw (unmerged) encounters to"
        )
        required.add_argument(
            "--vessel_id_table",
            required=True,
            action="append",
            dest="vessel_id_tables",
        ...
        required.add_argument("--start_date", required=True, help="First date to merge.")
        required.add_argument("--end_date", required=True, help="Last date (inclusive) to merge.")
        required.add_argument(
        ...
        optional.add_argument(
            "--wait_for_job", action="store_true", help="Wait for Dataflow to complete."
        )
        optional.add_argument(
            "--min_encounter_time_minutes",
            required=False,
            type=float,
        ...
        optional.add_argument(
            "--bad_segs_table", help="table of containing segment ids of bad segments"
        )
        optional.add_argument(
            "--ssvid_filter",
